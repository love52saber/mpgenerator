    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                // query.java
                if (null != tableInfo.getQueryName() && null != pathInfo.get(ConstVal.QUERY_PATH)) {
                    String queryFile = String.format((pathInfo.get(ConstVal.QUERY_PATH) + File.separator + tableInfo.getQueryName() + suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.QUERY, queryFile)) {
                        writer(objectMap, templateFilePath(template.getQuery()), queryFile);
                    }
                }
                // constant.java
                if (null != tableInfo.getConstantName() && null != pathInfo.get(ConstVal.CONSTANT_PATH)) {
